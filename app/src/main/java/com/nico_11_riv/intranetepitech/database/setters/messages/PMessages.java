    ...
    public void init(String api) {
        ...
        try {
            ...
            for (int i = 0; i < messages.length(); ++i) {
                ...
                Messages msg;
                m = Messages.findWithQuery(Messages.class, "SELECT * FROM Messages WHERE login = ? AND idmessage = ?", user.getLogin(), !Objects.equals(tmp.getString("id"), "null") ? tmp.getString("id") : "n/a");
                if (m.size() > 0)
                    msg = m.get(0);
                else
